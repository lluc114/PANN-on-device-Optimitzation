...
class NetBuilder:
    ...
    def __init__(self, model_path, 
            ...
            header_file="mlp_hidden_layer_pruned.h", #lcc
            out_directory='out',
            compiler='gcc'):
        ...
        self.header_path = os.path.join(TEMPLATE_DIR, header_file) #lcc
    ...
    def _load_header(self): #lcc
        self.header = open(self.header_path, 'r').read()
    ...
    def _dump_code(self):
        ...
        f = open(os.path.join(self.out_directory, 'main.h'), 'w')
        f.write(self.header)
        f.close()
...
class PrunedMLPBuilder(NetBuilder):
    ...
    def __init__(self, model_path, model_format='saved_model', out_directory='out', compiler='gcc'):
        ...
        Args:
            ...
            out_directory (str, optional): Output directory
        """
        super().__init__(model_path, model_format, out_directory=out_directory, compiler=compiler)
    ...
    def generate(self, x, half_precision=True, disable_prints=False, use_tanh=False):
        ...
        if self.model.layers[0].__class__.__name__ == 'Flatten':
        ...
        # n_layers = len(self.model.layers)
        # n_activs_h = np.zeros(n_layers, dtype=np.uint16)
        # use_bias_h = np.zeros(n_layers, dtype=np.uint8)
        # n_weights_h = np.zeros(n_layers, dtype=np.uint16)
        n_outputs = self.model.layers[-1].get_weights()[0].shape[1]
        ...
        for i, layer in enumerate(self.model.layers):
            ...
            if layer.__class__.__name__ != 'Dense': # If flatten layer
                if i > 1:
                    raise NotImplementedError
                continue
        ...
        self._replace_header('<N_INPUTS>', str(n_inputs))
        self._replace_header('<N_ACTIVS_H_TOTAL>', str(n_activs_h_total))
        self._replace_header('<N_HIDDENS>', str(n_layers))
        # self._replace('<N_ACTIVS_H1>', str(n_activs_h1))
        # self._replace('<N_ACTIVS_H2>', str(n_activs_h2))
        self._replace_header('<N_OUTPUTS>', str(n_outputs))
        # self._replace('<N_WEIGHTS_H1>', str(n_weights_h1))
        # self._replace('<N_WEIGHTS_H2>', str(n_weights_h2))
        self._replace_header('<NEURON_INPUTS>', self._array2str(neuron_inputs))
        self._replace_header('<INPUT_INDICES>', self._array2str(wi_pruned))
        ...
        self._replace_header('<N_WEIGHTS_H>', self._array2str(n_weights_h))
        self._replace_header('<USE_BIAS_H>', self._array2str(use_bias_h))
        self._replace_header('<N_ACTIVS_H>', self._array2str(n_activs_h))
        self._replace_header('<WEIGHTS_H>', self._array2str(w_pruned))
        self._replace_header('<BIAS_H>', self._array2str(b))
        self._replace_header('<USE_TANH>', str(use_tanh*1))
        ...
        self._replace_header('<WEIGHTS_CTYPE>', weights_ctype)
        self._replace_header('<BIAS_CTYPE>', bias_ctype)
        self._replace_header('<ACTIVATIONS_CTYPE>', activations_ctype)
